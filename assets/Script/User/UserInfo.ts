import { _decorator, ccclass, property, Component, Node, EventTarget, sys, macro, director, game, systemEvent } from 'cc';
...
const { ccclass: ccClass } = _decorator;
...
export default class UserInfo extends Component {
    ...
    public static setInstance(userData: any, token: string, exceptionAuth: any): boolean {
        ...
        const userInfoData = this.createUserInfoData();
        ...
        return userInfoData.userMasterInfo!.statusInfo.status === 0;
    }
    ...
    private updateFavoriteSlot(): void {
        const favSlots = ServerStorageManager.getAsStringArray(ServerStorageManager.StorageKeyType.SLOT_FAVORITE);
        ...
        ServerStorageManager.save(ServerStorageManager.StorageKeyType.SLOT_FAVORITE, favSlots);
    }
    ...
    public appsflyerTrackPurchaseAndroid(popupType: string, productId: string, money: number): void {
        if (SDefine.Mobile_AF_PurchaseConnector_Use ===1) return;
        if (SDefine.Mobile_AF_PurchaseLog_Revenue_Rounding ===1) {
    ...
    public async asyncPlaystoreExceptionBuyProduct_Renewal(data: any): Promise<void> {
        ...
        const payload = CommonServer.PurchasePayload_SmallInfo.parseObj(JSON.parse(data.obfuscatedAccountId));
        ...
        if (CommonServer.isServerResponseError(res)) {
            ...
            if (CommonServer.getErrorCode(res) === SDefine.ERR_PAYMENT_ALREADY_EXIST) {
                ...
                FireHoseSender.Instance().sendAws(FireHoseSender.Instance().getRecord(FireHoseSender.FHLogType.Trace, logDup), true, FireHoseSender.FHLogType.Trace);
        ...
        IAPManager_AOS.Instance().consume(data.purchaseToken);
        const changeRes = UserInfo.instance()!.getServerChangeResult(res);
        await this.asyncOpenUnProcessedInfoPopup(CommonServer.UnProcessedPopupInfo.newInst(payload.getProductId(), data.purchaseTime, changeRes));
    }
    ...
    public setTripleDiamondWheelGaugeInfo(info: any): void {
        ...
        this._eventEmitter.emit(ServiceInfoManager.MSG.UPDATE_TRIPLEWHEELGAUGE);
    }
    public getTripleDiamondWheelGaugeInfo(type: number): any {
        return type === SDefine.default.MAJORROLLER_ZONEID ? this._userInfo.tripleDiamondWheelInfo.goldGaugeInfo : this._userInfo.tripleDiamondWheelInfo.diamondGaugeInfo;
    }
    public setThrillJackpotWheelGaugeInfo(info: any): void {
        if (info) {
            ...
            this._eventEmitter.emit(ServiceInfoManager.MSG.UPDATE_THRILLWHEELGAUGE);
        }
    }
    ...
    public getTripleDiamondWheelGoldAmount(): number {
        const items = this._userInfo.userInven.getItemsByItemId(SDefine.default.I_TRIPLEDIA_JACKPOT_GOLD_TICKET);
        return items.length <= 0 ? 0 : items[0].curCnt;
    }
    public getStarShopPoint(): number {
        const items = this._userInfo.userInven.getItemsByItemId(SDefine.default.I_STAR_SHOP_COIN);
        return items.length === 0 ? 0 : items[0].curCnt;
    }
    public getSuiteLeagueBalance(): number {
        const items = this._userInfo.userInven.getItemsByItemId(SDefine.default.I_SUITE_LEAGUE_SHOP_BALANCE);
        return items.length === 0 ? 0 : items[0].curCnt;
    }
    public getTripleDiamondWheelDiaAmount(): number {
        const items = this._userInfo.userInven.getItemsByItemId(SDefine.default.I_TRIPLEDIA_JACKPOT_DIAMOND_TICKET);
        return items.length <= 0 ? 0 : items[0].curCnt;
    }

    public applyHeroBonusGauge(info: any): void {
        if (!this._userHeroInfo) {
            error("applyHeroBonusGauge invalid heroStatus");
            return;
        }
        const oldGrade = HeroManager.default.Instance().getBonusGaugeGrade(this._userHeroInfo.getActiveHeroBonusGauge());
        this._userHeroInfo.addHeroBonusGauge(info.addGauge);
        const newGrade = HeroManager.default.Instance().getBonusGaugeGrade(this._userHeroInfo.getActiveHeroBonusGauge());
        if (oldGrade !== newGrade) {
            this._eventEmitter.emit(ServiceInfoManager.MSG.UPDATE_HERO_BONUS_GRADEUP, info);
        } else {
            this._eventEmitter.emit(ServiceInfoManager.MSG.UPDATE_HERO_BONUS_EXPUP, info);
        }
    }
