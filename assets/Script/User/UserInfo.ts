...
export default class UserInfo extends cc.Component {
    ...
    public addUserAssetMoney(val: number): void {
        ...
        const oldCoin = this._totalCoin;
        this._totalCoin += val;
        if (this._totalCoin < 0) {
            console.error(`addUserAssetMoney fail...${val.toString()} - ${oldCoin.toString()}`);
            this._totalCoin = 0;
        }
        // this._userInfo!.userAssetInfo!.biggestTotalCoin = Math.max(this._totalCoin, this._userInfo!.userAssetInfo!.biggestTotalCoin);
    ...
    public async asyncRefreshJackpotInfoByZoneId(zoneId: number) {
        const res = await CommonServer.Instance().requestZoneInfo() as any;
