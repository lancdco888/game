...
export default class LobbyScene extends cc.Component {
    ...
    public static BackProcess(): void {
        if (0 != Utility.isMobileGame()) {
            PopupManager.default.Instance().showDisplayProgress(true);
            CommonPopup.default.getCommonPopup((isValid, popup) => {
                PopupManager.default.Instance().showDisplayProgress(false);
    ...
    public get isPassableHigh(): boolean {
        return UserInfo.default.instance().isPassAbleCasino(SDefine.VIP_LOUNGE_ZONEID, SDefine.VIP_LOUNGE_ZONENAME);
    }
    public get isPassableDynamic(): boolean {
        return UserInfo.default.instance().isPassAbleCasino(SDefine.SUITE_ZONEID, SDefine.SUITE_ZONENAME);
    }
    ...
    onLoad(): void {
        ...
        // 用户场景状态赋值
        UserInfo.default.instance().setCurrentSceneMode(SDefine.Lobby);
        LobbyScene._instance = this;

        // 消息监听注册 - 大厅步骤切换/弹窗重置
        MessageRoutingManager.default.instance().addListenerTarget(MessageRoutingManager.default.MSG.LOBBY_NEXT_STEP, this.openLobbyStartPopup, this);
        MessageRoutingManager.default.instance().addListenerTarget(MessageRoutingManager.default.MSG.RESET_START_POPUP, this.resetOpenLobbyStartPopup, this);
    }
    ...
    onDestroy(): void {
        ...
        MessageRoutingManager.default.instance().removeListenerTargetAll(this);
        
        if (TSUtility.isValid(UserInfo.default.instance())) {
            UserInfo.default.instance().removeListenerTargetAll(this);
        }
    ...
    public initialize(): Promise<void> {
        return new Promise(async (resolve) => {
            try {
                const loadingPopup = LoadingPopup.default.getCurrentOpenPopup();
                loadingPopup.setPostProgress(0, "Authenticating ...");
                
                // 初始化埋点+用户信息重置
                Analytics.default.lobbyInitStartInit();
                const userInstance = UserInfo.default.instance();
                ...
                if (ServiceInfoManager.default.STRING_LAST_LOBBY_NAME == SDefine.SUITE_ZONENAME && this.isPassableDynamic) {
                ...
                // 弹窗返回键绑定退出逻辑
                PopupManager.default.Instance().setBaseBackBtnCallback(LobbyScene.BackProcess);
                ServiceInfoManager.default.NUMBER_LOOBY_ENTER_COUNT++;
                if (ServiceInfoManager.default.NUMBER_START_MEMBERS_LEVEL < 0) {
                    ServiceInfoManager.default.NUMBER_START_MEMBERS_LEVEL = userInstance.getUserVipInfo().level;
                }

                // 进度条更新 20%
                Analytics.default.customLoadingRecord("lob_setUi_complete_" + (ServiceInfoManager.default.NUMBER_LOOBY_ENTER_COUNT - 1).toString());
                loadingPopup.setPostProgress(0.2, "Verifying game ...");
                this.refreshBGM();

                // 移动端新手引导标记
                if (1 == Utility.isMobileGame() && 0 == ServerStorageManager.default.getAsBoolean(ServerStorageManager.default.StorageKeyType.MOBILE_GUIDE)) {
                    ServerStorageManager.default.save(ServerStorageManager.default.StorageKeyType.MOBILE_GUIDE, true);
                }
                ...
                if (!TSUtility.isValid(slotZoneInfo)) {
                    ...
                    Analytics.default.lobbyInitStartRefreshJackpotInfo();
                    const refreshJackpotRes = await userInstance.asyncRefreshJackpotInfo(true);
                    Analytics.default.lobbyInitCompleteRefreshJackpotInfo();
                        ...
                        CommonPopup.default.getCommonPopup((isValid, popup) => {
                            ...
                            if (!isValid) {
                                popup.open()
                                    ...
                                    .setOkBtn(1 == Utility.isFacebookInstant() ? "CLOSE" : "RETRY", () => { HRVServiceUtil.default.restartGame(); });
                ...
                PowerGemManager.default.instance.initialize();
                TimeBonusManager.default.instance.initialize();
                SupersizeItManager.default.instance.initialize();

                // 大厅移动管理器初始化
                Analytics.default.lobbyInitStartLobbyMoveManager();
                this._mgrLobbyUIMove = new LobbyMoveManager();
                await this._mgrLobbyUIMove.initialize();
                Analytics.default.lobbyInitCompleteLobbyMoveManager();

                // 大厅启动弹窗管理器初始化
                Analytics.default.lobbyInitStartStartPopupManager();
                this._mgrLobbyUIStartPopup = new LobbyUIStartPopupManager();
                await this._mgrLobbyUIStartPopup.initialize();
                Analytics.default.lobbyInitCompleteStartPopupManager();
                ...
                // 每日充值奖励校验
                Analytics.default.lobbyInitStartCheckDailyTopupReward();
                await userInstance.asyncCheckDailyTopUpReward(null);
                Analytics.default.lobbyInitCompleteCheckDailyTopupReward();

                // 数据刷新埋点 + 卷轴任务版本更新
                Analytics.default.lobbyInitStartCheckAndRefreshInfos();
                Analytics.default.lobbyInitCompleteCheckAndRefreshInfos();
                Analytics.default.lobbyInitStartUpdateReelQuestVersion();
                await this.asyncUpdateReelQuestVersion();
                Analytics.default.lobbyInitCompleteUpdateReelQuestVersion();

                // 狂热模式门票使用 + FB小队信息刷新
                SlotFeverModeManager.default.instance.useFeverTicket(() => {});
                Analytics.default.lobbyInitAfterUseFeverTicket();
                if (SDefine.FBInstant_Squad_Use && TSUtility.isValid(FBSquadManager.default.Instance().getSquadInfo())) {
                    FBSquadManager.default.Instance().refreshSquadInfo();
                }
                ...
                // 进度条100%完成 + 入场动画播放
                Analytics.default.lobbyInitAfterCompleteDelay();
                loadingPopup.setPostProgress(1, "Completed", true);
                await this.UI.playEnterAction();

                // 首次进入校验未处理的内购订单
                const enterLobbyCnt = ServiceInfoManager.default.NUMBER_LOOBY_ENTER_COUNT;
                const enterSlotCnt = ServiceInfoManager.default.NUMBER_SLOT_ENTER_COUNT;
                if (enterLobbyCnt + enterSlotCnt <= 1) {
                    Analytics.default.lobbyInitStartCheckUnprocessedPurchase();
                    await UnprocessedPurchaseManager.default.Instance().doProcess();
                    Analytics.default.lobbyInitCompleteCheckUnprocessedPurchase();
                }

                // 初始化完成收尾逻辑
                const completeInit = () => {
                    if (0 == ServerStorageManager.default.getAsBoolean(ServerStorageManager.default.StorageKeyType.FIRST_VISIT_LOBBY)) {
                        ServerStorageManager.default.save(ServerStorageManager.default.StorageKeyType.FIRST_VISIT_LOBBY, true);
                    }
                    Analytics.default.lobbyInitCompleteAll();
                    Analytics.default.enterCasinoComplete(this._numZoneID);
                    ServiceInfoManager.default.STRING_SNEAK_PEEK_GAME_ID = "";
                    this.openLobbyStartPopup();
                };

                // 从老虎机返回大厅时的广告逻辑
                if ("Slot" == userInstance.getPrevLocation() && 1 == AdsManager.default.Instance().isUseable()) {
                    Analytics.default.lobbyInitPrevLocationSlot();
                    AdsManager.default.Instance().ADLog_InterstitialShowUI(AdsManager.default.PlacementID_InterstitalType.SLOTTOLOBBY);
                    if (ADTargetManager.default.instance().enableInterstitialAD()) {
                        AdsManager.default.Instance().InterstitialAdplay(AdsManager.default.PlacementID_InterstitalType.SLOTTOLOBBY, () => {
                            ServiceInfoManager.default.instance().addInterstitialADPlayCount();
                            if (1 == ServiceInfoManager.default.instance().getShowADSFreePopup()) {
                        ...
                        if (1 == ADTargetManager.default.instance().enableInterstitialAD(false)) {
                            cc.log("Lobby Check enableInterstitialAD true");
                            AdsManager.default.Instance().preloadInterstitialAD();
                        }
                        completeInit();
                    }
                } else {
                    Analytics.default.lobbyInitPrevLocationLobby();
                    completeInit();
                }
            } catch (error) {
                // 异常上报
    ...
    public resetOpenLobbyStartPopup(): void {
        ...
        PopupManager.default.Instance().showDisplayProgress(false);
        ServiceInfoManager.default.BOOL_OPENING_LOBBY_POPUP = false;
        
        const couponUI = this.UI.getLobbyUI(LobbyUIBase.LobbyUIType.COUPON);
    ...
    public refreshBGM(): void {
        if (!this._isOverrideBGM) {
            const jackpotInfo = SlotJackpotManager.default.Instance().getZoneJackpotInfo(this._numZoneID);
    ...
    public playBGM(burningState: number): void {
        if (!this._isOverrideBGM) {
            const activeSkinList = SkinInfoManager.default.Instance().getActiveSkinInfoList();
            ...
            if (TSUtility.isValid(activeSkinList) && activeSkinList.length > 0) {
                for (let i = 0; i < activeSkinList.length; i++) {
                    if (1 == activeSkinList[i].key.toUpperCase().includes("CHRISTMAS")) {
            ...
            if (this._strCurBGMName != bgmName) {
                ...
                SoundManager.default.Instance().playBGM(this.soundSetter.getAudioClip(this._strCurBGMName));
                SoundManager.default.Instance().setMainVolumeTemporarily(1);
            }
        }
    }
            ...
            await new Promise((innerResolve, innerReject) => {
                ...
                CommonServer.Instance().requestAcceptPromotion(
                    ...
                    (response) => {
                        ...
                        CommonServer.default.Instance().requestReelQuestRefresh((questRes) => {
                            PopupManager.default.Instance().showDisplayProgress(false);
                            if (CommonServer.default.isServerResponseError(questRes)) {
                                innerReject();
                            } else {
    ...
    public tourneyLobbySchedule(): Promise<void> {
        return new Promise(async (resolve) => {
            ...
            try {
                ...
                const tourneyInfo = SlotTourneyManager.default.Instance().getCurrentTourneyInfo();
                const currentUnix = TSUtility.getServerBaseNowUnixTime();
                
                // 锦标赛信息刷新校验
                if (SlotTourneyManager.default.Instance().getLastUpdateTimeProgressInfo(0) + 60 <= currentUnix && tourneyInfo.getNextSlotStartTime() > currentUnix + 60) {
                    await this.asyncRefreshTourneyInfo();
                }

                // 锦标赛参与完成校验
                const isComplete = await SlotTourneyManager.default.Instance().asyncCheckCompleteParticipateTourney(false, -1, -1);
                if (!TSUtility.isValid(this)) { resolve(); return; }
                if (1 == isComplete) {
                    await HeroTooltipPopup.default.asyncGetTourneyCompleteTooltip(this.node, this.UI.getLobbyNode(LobbyUIBase.LobbyUIType.MENU));
                }
            } catch (error) {
    ...
    public asyncTourneyLobbyWork(): Promise<void> {
        return new Promise(async (resolve) => {
            ...
            if (tourneyInfo.getNextSlotStartTime() <= currentUnix) {
                ...
                MessageRoutingManager.default.instance().emitMessage(MessageRoutingManager.default.MSG.TOURNEY_RESERVE_NEWGAME);
                const tourneyRes = await CommonServer.default.Instance().asyncRequestSlotTourneyInfo();
                
                if (!TSUtility.isValid(this)) { resolve(); return; }
                if (1 == CommonServer.default.isServerResponseError(tourneyRes)) {
                ...
                SlotTourneyManager.default.Instance().setSlotTourneyMasterInfo(tourneyMaster);
                if (TSUtility.isValid(tourneyRes.slotTourneyProgressInfos)) {
                    for (let i = 0; i < tourneyRes.slotTourneyProgressInfos.length; ++i) {
                        ...
                        SlotTourneyManager.default.Instance().setSlotTourneyProgressInfo(progressInfo);
                    }
                }
                MessageRoutingManager.default.instance().emitMessage(MessageRoutingManager.default.MSG.TOURNEY_START_NEWGAME);
            }
            resolve();
        });
    }
    ...
    public asyncRefreshTourneyInfo(): Promise<void> {
        return new Promise(async (resolve) => {
            ...
            MessageRoutingManager.default.instance().emitMessage(MessageRoutingManager.default.MSG.TOURNEY_REFRESH_INFO);
